  ...
  <script>
    document.addEventListener('alpine:init', () => {
      Alpine.data('reader', () => ({
        ...
        loadSub(c,id){
          const index = this.subchapters.findIndex(s => s.id === id);
          this.activeSubIndex = index;
          this.activeSub = id;

          const sub = this.subchapters[index];

          // Buka bab parent di sidebar
          if(sub.chapterIndex !== -1 && !this.openIndexes.includes(sub.chapterIndex)){
            this.openIndexes.push(sub.chapterIndex);
          }
          ...
          this.$nextTick(() => {
            document.querySelector('article')?.scrollTo({top:0});
            const activeLink = document.querySelector('[data-subid="'+id+'"]');
        ...
        updateContent(){
          ...
          this.content = `<img src='{{ asset('storage') }}/`+p+`'
                           class='w-full h-auto shadow-orange-700 rounded-lg shadow-md'>`;
        },

        goNext(){
          if(this.contentImages.length > 0 && this.currentImageIndex < this.contentImages.length - 1){
            this.currentImageIndex++;
            this.updateContent();
          } else {
            if(this.activeSubIndex < this.subchapters.length - 1){
              this.activeSubIndex++;
              const nextSub = this.subchapters[this.activeSubIndex];
              this.loadSub(nextSub.content, nextSub.id);
            }
          }
        },

        goPrev(){
          if(this.contentImages.length > 0 && this.currentImageIndex > 0){
            this.currentImageIndex--;
            this.updateContent();
          } else {
            if(this.activeSubIndex > 0){
              this.activeSubIndex--;
              const prevSub = this.subchapters[this.activeSubIndex];
              this.loadSub(prevSub.content, prevSub.id);

              if (Array.isArray(prevSub.content)) {
                this.currentImageIndex = prevSub.content.length - 1;
                this.updateContent();
              }
            }
          }
...
<body class="bg-gray-100 text-gray-900 font-sans">
  ...
  <div class="flex h-screen overflow-hidden" x-data="reader">
      ...
      <nav class="space-y-3">
        @foreach($ebook->chapters as $i => $chapter)
          <div class="border-b border-gray-300 pb-3">
            ...
            <div x-show="isOpen({{ $i }})" x-transition class="mt-2 space-y-1" id="chapter-{{ $i }}">
              @foreach($chapter->subchapters as $sub)
                <a
                  href="#"
                  :data-subid="'{{ $sub->id }}'"
    ...
    <!-- Konten -->
    <main class="flex-1 flex flex-col items-center overflow-y-auto p-6 bg-gray-50" style="min-width: 0;" aria-live="polite">
      <article
      ...
      <div class="mt-6 flex justify-between max-w-4xl w-full gap-4">
        <button
          ...
          :disabled="activeSubIndex <= 0 && currentImageIndex <= 0"
          @click="goPrev"
        ...
        <button
          class="flex items-center justify-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
          :disabled="activeSubIndex >= subchapters.length - 1 && currentImageIndex >= contentImages.length - 1"
          @click="goNext"
